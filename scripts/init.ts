...
if (serverConfig.cloud === undefined) {
    // PEM-Dateien fuer HTTPS kopieren
    const keyPemSrc = join(configSrc, 'key.pem');
    copyFileSync(keyPemSrc, join(configDist, 'key.pem'));

    // Zertifikatdateien fuer HTTPS kopieren
    const certificateSrc = join(configSrc, 'certificate.cer');
    copyFileSync(certificateSrc, join(configDist, 'certificate.cer'));

    // Zertifikatdatei fuer MongoDB kopieren
    const mongoSrc = join(configSrc, 'mongodb.cer');
    copyFileSync(mongoSrc, join(configDist, 'mongodb.cer'));
}
